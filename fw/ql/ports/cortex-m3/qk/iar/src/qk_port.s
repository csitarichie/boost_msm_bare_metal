...
; exception). In QK, this is exactly the time when the QK scheduler
; needs to check for the asynchronous preemptions.
;
;*****************************************************************************
QK_PendSV
    LDR     r0,=QK_readySet_  ; load the address of QK_readySet_
    LDRB    r0,[r0]           ; load the value of QF_readySet_
    CBZ     r0,iret           ; if QK_readySet_ == 0, return from interrupt

    MOV     r1,#0x01000000    ; make up a task xPSR with only the T bit set
    LDR     r0,=schedule      ; load the address of scheduler wrapper (new PC)
    PUSH    {r0-r1}           ; push xPSR,PC
    SUB     sp,sp,#(6*4)      ; don't care for lr,r12,r3,r2,r1,r0
iret
    BX      lr                ; interrupt return to the task or scheduler
